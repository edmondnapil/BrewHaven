...
// Admin sees admin + customer login history, never Super Admin's.
$roleScope = ['admin', 'customer'];
$allSessions = bh_build_login_sessions($conn, $roleScope, null, '', '', $search);
$availableMonths = [];
...
$sessions = bh_build_login_sessions($conn, $roleScope, null, $month, $date, $search);
...
<head>
  ...
  <title>Login Logs - Brew Haven Admin</title>
...
<body class="bh-shell">
  ...
  <div class="bh-layout">
    ...
    <div class="bh-main">
      ...
      <p style="color:var(--bh-text-muted);margin-top:0.4rem;margin-bottom:1.2rem;">Time in / time out history for Admin and Customer accounts &middot; <?php echo $totalRows; ?> session<?php echo $totalRows === 1 ? '' : 's'; ?> match this filter</p>

      <div class="bh-panel">
        <form method="get" action="admin-login-logs.php" class="bh-toolbar" style="flex-wrap:wrap;">
